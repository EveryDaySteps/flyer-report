#!/usr/bin/env python3
"""
generate_flyer_keyword_report_v5_fixed.py
...
def load_store_list(stores_json_path: Path | None) -> list[dict[str, str]]:
    if not stores_json_path or not stores_json_path.exists():
        return []
    ...
    for item in data:
        ...
        url = str(item.get("url") or item.get("store_url") or "").strip()
        ...
        if key in seen:
            continue
        seen.add(key)
        stores.append({"name": name or "店舗名未設定", "url": url})
    return stores
...
.store-link-bar{{margin:10px 0 4px;display:flex;justify-content:flex-end}}
.store-list-link{{border:1px solid var(--line);background:white;color:var(--brand);border-radius:999px;padding:8px 12px;font-weight:900;cursor:pointer;text-decoration:none;box-shadow:var(--shadow)}}
.store-list-panel{{background:white;border:1px solid var(--line);border-radius:18px;box-shadow:var(--shadow);padding:14px;margin:12px 0}}
.store-list-panel h2{{margin:0 0 6px;font-size:18px}}
.store-list-panel p{{margin:0 0 12px;color:var(--muted);font-size:12px}}
.store-table{{width:100%;border-collapse:collapse;background:white;overflow:hidden;border-radius:12px}}
.store-table th,.store-table td{{border-bottom:1px solid var(--line);padding:10px;text-align:left;font-size:13px;vertical-align:top}}
.store-table th{{color:#344054;background:#f8fafc;font-size:12px}}
.store-table a{{color:var(--brand);word-break:break-all}}
...
<div class="store-link-bar"><button class="store-list-link" onclick="showStoreList()">チラシ確認店舗一覧</button></div>
...
<section id="storeListView" class="hidden"><div class="store-list-panel"><h2>チラシ確認店舗一覧</h2><p>このレポート作成時に確認対象としている店舗です。検出結果が0件の店舗も表示します。</p><div id="storeListBody"></div></div></section>
...
const STORE_LIST = {store_list_json};
let currentZoom = 1.0;
...
function renderStoreList(){{
 const root=document.getElementById('storeListBody');
 if(!root) return;
 if(!STORE_LIST.length){{root.innerHTML='<div class="empty">stores.jsonが見つからないか、店舗情報がありません。</div>';return;}}
 root.innerHTML=`<table class="store-table"><thead><tr><th>店舗名</th><th>店舗URL</th></tr></thead><tbody>${{STORE_LIST.map(s=>`<tr><td>${{esc(s.name)}}</td><td>${{s.url?`<a href="${{esc(s.url)}}" target="_blank" rel="noopener noreferrer">${{esc(s.url)}}</a>`:'-'}}</td></tr>`).join('')}}</tbody></table>`;
}}
function showStoreList(){{
 showView('storeListView');
 document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));
}}
function showView(id){{document.querySelectorAll('.tab').forEach(t=>t.classList.toggle('active',t.dataset.view===id));document.getElementById('storeListView').classList.toggle('hidden',id!=='storeListView');document.getElementById('summaryView').classList.toggle('hidden',id!=='summaryView');document.getElementById('detailView').classList.toggle('hidden',id!=='detailView');if(id==='detailView'&&pendingFocusHit){{setTimeout(()=>focusHit(pendingFocusHit),120);}}}}
...
document.querySelectorAll('.tab').forEach(t=>t.addEventListener('click',()=>showView(t.dataset.view)));['search','storeFilter','categoryFilter'].forEach(id=>document.getElementById(id).addEventListener('input',renderSummary));renderStoreList();renderSummary();renderDetails();setupPinchZoom();setZoom(1);
</script></body></html>'''
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate flyer keyword HTML report v5 fixed.")
    ...
    parser.add_argument("--stores-json", default="stores.json", help="確認対象店舗一覧のJSON。既定は ./stores.json")
    args = parser.parse_args()

    output_dir = Path(args.output_dir)
    stores_json_path = Path(args.stores_json)
    report = build_report(output_dir, args.report_name, stores_json_path=stores_json_path)
    print(f"HTML: {report}")
