...
def load_store_config(path: Path | None) -> dict[str, dict[str, Any]]:
    """
    店舗ごとの検索カテゴリ設定を読み込む。

    対応形式:
    [
      {"name": "カスミ結城店", "categories": ["アイス"]},
      {"name": "西松屋筑西川島店", "categories": ["おむつ"]}
    ]

    または:
    {"stores": [...]}

    categories がない/空の場合は「全カテゴリ検索」として扱う。
    """
    if not path:
        return {}
    if not path.exists():
        raise ValueError(f"Store config not found: {path}")
    ...
    if not isinstance(data, list):
        raise ValueError("store config must be a list or {'stores': [...]}")
    ...
    for item in data:
        ...
        if categories is None:
            ...
            raise ValueError(f"categories must be a list: {name}")
...
def filter_keywords_for_store(
    all_keywords: dict[str, dict[str, list[str]]],
    store_name: str | None,
    store_config: dict[str, dict[str, Any]],
) -> dict[str, dict[str, list[str]]]:
    """
    店舗名に対応する categories があれば、そのカテゴリだけに絞る。
    categories 未指定/空の場合は全カテゴリ検索。
    """
    if not store_name:
        return all_keywords

    conf = store_config.get(store_name)
    if not conf:
        return all_keywords
    ...
    unknown = [c for c in categories if c not in all_keywords]
    if unknown:
        print(f"  WARNING: store config has unknown category for {store_name}: {', '.join(unknown)}")

    return {
        category: all_keywords[category]
...
def process_image(image_path: Path, output_dir: Path, keywords: dict[str, dict[str, list[str]]], api_key: str, args: argparse.Namespace, store_name: str | None = None) -> list[KeywordHit]:
    img = Image.open(image_path).convert("RGB")
    W, H = img.size
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Find flyer keywords using OCR.space. v6 with per-store category filtering.")
    parser.add_argument("input", help="Input image file or directory")
    parser.add_argument("--keywords", default="./flyer_keywords.yaml", help="Keyword YAML file")
    parser.add_argument("--output-dir", default="./keyword_output_v6")
    parser.add_argument("--store-name", default=None, help="指定した場合は全画像にこの店舗名を設定。未指定なら親フォルダ名を店舗名として自動設定します。")
    parser.add_argument("--store-config", default=None, help="店舗ごとの検索カテゴリ設定JSON。例: flyer_stores.json")
    ...
    store_config = load_store_config(Path(args.store_config)) if args.store_config else {}
    ...
    print(f"Store config: {args.store_config if args.store_config else '(none)'}")
    ...
    for image_path in images:
        ...
        try:
            store_keywords = filter_keywords_for_store(keywords, store_name, store_config)
            print(f"  Search categories: {', '.join(store_keywords.keys()) if store_keywords else '(none)'}")
            if not store_keywords:
                print("  -> 0 hit(s) [no categories to search]")
                continue
